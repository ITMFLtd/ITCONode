...
void rai::uint256_union::encode_account (std::string & destination_a) const
{
	...
	destination_a.append ("_octi"); // xrb_ itco_
...
bool rai::uint256_union::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		auto xrb_prefix (source_a[0] == 'i' && source_a[1] == 't' && source_a[2] == 'c' && (source_a[3] == '_' || source_a[3] == '-'));
		auto nano_prefix (source_a[0] == 'i' && source_a[1] == 't' && source_a[2] == 'c' && source_a[3] == 'o' && (source_a[4] == '_' || source_a[4] == '-'));
